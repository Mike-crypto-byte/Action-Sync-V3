...
const CrapsGame = ({ onBack }) => {
  ...
  const ADMIN_PASSWORD = 'dealer2024'; // CHANGE THIS TO YOUR OWN PASSWORD!
  ...
  const [bonusRecipient, setBonusRecipient] = useState('all'); // 'all' or specific userId
  
  // Game state
  const [gameMode, setGameMode] = useState('standard'); // 'standard' or 'crapless'
  const [gamePhase, setGamePhase] = useState('come-out'); // 'come-out' or 'point'
  ...
  const loadGameState = async () => {
    try {
      const state = await window.storage.get('game-state', true);
      if (state) {
        const data = JSON.parse(state.value);
        setGamePhase(data.gamePhase || 'come-out');
        setPoint(data.point || null);
        setRollNumber(data.rollNumber || 0);
  ...
  const saveGameState = async (updates) => {
    ...
    await window.storage.set('game-state', JSON.stringify(state), true);
  };
  ...
  const loadUserData = async () => {
    try {
      const userData = await window.storage.get(`user-${userId}`);
      if (userData) {
        const data = JSON.parse(userData.value);
        setUserName(data.name);
        setBankroll(data.bankroll);
        setIsRegistered(true);
        setActiveBets(data.activeBets || { passLine: { amount: 0, placed: false }, come: [] });
      }
    } catch (e) {
      // User not registered
    }
  ...
  const saveUserData = async (updates) => {
    ...
    await window.storage.set(`user-${userId}`, JSON.stringify(userData));
  };

  const updateLeaderboard = async (newBankroll) => {
    try {
      const current = await window.storage.get('leaderboard', true);
      ...
      await window.storage.set('leaderboard', JSON.stringify(leaders), true);
  ...
  const registerUser = async () => {
    if (userName.trim()) {
      ...
      const hasAdminAccess = adminPassword === ADMIN_PASSWORD;
      
      // Use custom starting chips if dealer set it, otherwise default to 1000
      const startingBankroll = hasAdminAccess ? 1000 : customStartingChips;
      ...
      await updateLeaderboard(startingBankroll);
    }
  };
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const current = await window.storage.get('chat-messages', true);
  ...
  const adminSubmitRoll = async () => {
    ...
    if (d1 >= 1 && d1 <= 6 && d2 >= 1 && d2 <= 6) {
      ...
      // Process roll for current user
      await resolveRoll(d1, d2);
  ...
  const distributeBonusChips = async () => {
    ...
    if (bonusRecipient === 'all') {
      ...
      if (confirm(`Give $${bonusChipsAmount.toLocaleString()} to ALL ${leaderboard.length} players?`)) {
        // Update all players in leaderboard
        for (const player of leaderboard) {
          try {
            const userData = await window.storage.get(`user-${player.userId}`);
            if (userData) {
              const data = JSON.parse(userData.value);
              const newBankroll = data.bankroll + bonusChipsAmount;
              await window.storage.set(`user-${player.userId}`, JSON.stringify({
                ...data,
                bankroll: newBankroll
              }));
            }
      ...
      if (confirm(`Give $${bonusChipsAmount.toLocaleString()} to ${targetPlayer.name}?`)) {
        try {
          const userData = await window.storage.get(`user-${bonusRecipient}`);
          if (userData) {
            const data = JSON.parse(userData.value);
            const newBankroll = data.bankroll + bonusChipsAmount;
            await window.storage.set(`user-${bonusRecipient}`, JSON.stringify({
              ...data,
              bankroll: newBankroll
            }));
  ...
  const adminResetSession = async () => {
    if (confirm('Reset entire session? This will clear all user data.')) {
      try {
        ...
        await window.storage.set('leaderboard', JSON.stringify([]), true);
        ...
        setCountdown(15);
        setBankroll(1000);
        setCurrentBets({ passLine: 0, dontPass: 0, come: 0, dontCome: 0 });
        setActiveBets({ passLine: { amount: 0, placed: false }, come: [] });
        
        await saveUserData({ bankroll: 1000, activeBets: { passLine: { amount: 0, placed: false }, come: [] } });
      } catch (e) {
  ...
  // Count active users from leaderboard
  useEffect(() => {
          ...
          <details style={{ marginTop: '25px' }}>
            ...
            <div style={{ marginTop: '15px' }}>
              ...
              <div style={{
                ...
                Leave blank to join as a player
              </div>
            </div>
          </details>
        </div>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1400px', margin: '20px auto', padding: '0 20px' }}>
        ...
        <div style={{
          ...
          {/* Main Betting Area */}
          <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr 120px', gap: '15px' }}>
            ...
            <div>
              ...
              <div style={{ 
                display: 'grid', 
                gridTemplateColumns: gameMode === 'crapless' ? 'repeat(10, 1fr)' : 'repeat(6, 1fr)',
                gap: '8px',
                marginBottom: '12px'
              }}>
                {gameMode === 'crapless' && (
