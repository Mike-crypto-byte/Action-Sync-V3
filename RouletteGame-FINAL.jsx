    ...
    if (bonusRecipient === 'all') {
      if (confirm(`Give $${bonusChipsAmount.toLocaleString()} to ALL ${leaderboard.length} players?`)) {
        for (const player of leaderboard) {
          try {
            const userData = await window.storage.get(`roulette-user-${player.userId}`);
            if (userData) {
              const data = JSON.parse(userData.value);
              const newBankroll = data.bankroll + bonusChipsAmount;
              await window.storage.set(`roulette-user-${player.userId}`, JSON.stringify({
                ...data,
                bankroll: newBankroll
              }));
            }
      ...
      if (confirm(`Give $${bonusChipsAmount.toLocaleString()} to ${targetPlayer.name}?`)) {
        try {
          const userData = await window.storage.get(`roulette-user-${bonusRecipient}`);
          if (userData) {
            const data = JSON.parse(userData.value);
            const newBankroll = data.bankroll + bonusChipsAmount;
            await window.storage.set(`roulette-user-${bonusRecipient}`, JSON.stringify({
              ...data,
              bankroll: newBankroll
            }));
