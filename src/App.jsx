...
const App = () => {
  ...
  // Dealer deactivates game — sends all players back to waiting screen
  const deactivateGame = async () => {
    try {
      const dbRef = ref(db, 'activeGame');
      await set(dbRef, null);
      setSelectedGame(null);
      console.log('🛑 Game deactivated — players return to waiting screen');
    } catch (e) {
  ...
  if (selectedGame === 'craps') {
    return <CrapsGame onBack={() => isDealerMode ? deactivateGame() : null} isDealerMode={isDealerMode} />;
  }

  if (selectedGame === 'baccarat') {
    return <BaccaratGame onBack={() => isDealerMode ? deactivateGame() : null} isDealerMode={isDealerMode} />;
  }

  if (selectedGame === 'roulette') {
    return <RouletteGame onBack={() => isDealerMode ? deactivateGame() : null} isDealerMode={isDealerMode} />;
  }
