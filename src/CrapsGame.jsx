...
const CrapsGame = ({ onBack, isDealerMode = false }) => {
  // User state
  const [userId] = useState(() => {
    let id = sessionStorage.getItem('craps-userId');
    if (!id) { id = `user_${Math.random().toString(36).substr(2, 9)}`; sessionStorage.setItem('craps-userId', id); }
    return id;
  });
  ...
  const defaultGameState = {
    ...
    lastRoll: null, bettingOpen: true, countdown: 15, rollHistory: []
  };
  ...
  const { chatMessages: fbChatMessages, sendMessage: fbSendMessage, clearChat } = useChat(GAME_NAME);
  ...
  useEffect(() => {
    if (userData && !isRegistered) {
      ...
      setSessionStats(userData.sessionStats || { totalWagered: 0, biggestWin: 0, totalRolls: 0, startingBankroll: 1000 });
    }
  }, [userData]);
  ...
  const confirmBets = async () => {
    ...
    // Show result banner
    const totalWagered = Object.values(activeBets).reduce((s, v) => s + v, 0);
  ...
  const adminResetSession = async () => {
    if (confirm('Reset entire session?')) {
      try {
        ...
        clearAllBets();
        setActiveBets({});
        setSessionStats({ totalWagered: 0, biggestWin: 0, totalRolls: 0, startingBankroll: 1000 });
        await saveUserData({ bankroll: 1000, activeBets: {}, sessionStats: { totalWagered: 0, biggestWin: 0, totalRolls: 0, startingBankroll: 1000 }});
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '15px', marginBottom: '12px' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '9px', color: '#666', marginBottom: '4px', textAlign: 'center' }}>DIE 1</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '4px' }}>
                    {[1,2,3,4,5,6].map(n => (
                      <button key={`d1-${n}`} onClick={() => setAdminDice1(String(n))}
                        style={{
                          padding: '12px 0', borderRadius: '6px', fontSize: '18px', fontWeight: 'bold',
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '9px', color: '#666', marginBottom: '4px', textAlign: 'center' }}>DIE 2</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '4px' }}>
                    {[1,2,3,4,5,6].map(n => (
        ...
        {resultBanner && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                fontSize: '36px', fontWeight: 'bold', color: '#fff', marginBottom: '5px'
              }}>
                {resultBanner.type === 'win' ? '+' : ''}{resultBanner.amount >= 0 ? '+' : ''}${Math.abs(resultBanner.amount).toLocaleString()}
              </div>
