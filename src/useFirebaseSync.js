...
//    Archives previous session → resets bankrolls → clears live session.
//    Sets status to "waiting" — dealer then hits Start Stream to go live.
// ============================================================
...
// ============================================================
// 8b. startStream — dealer goes live within an existing session.
//     Just flips status to "active" — does NOT reset bankrolls or
//     wipe anything. Call this when the dealer is ready to start
//     taking bets, before selecting the first game of the session.
// ============================================================
...
// ============================================================
// 8c. switchGame — dealer changes active game mid-session.
//     Clears the previous game's state so stale results don't
//     carry over, then sets the new activeGame.
// ============================================================
export async function switchGame(dealerUid, newGame) {
  if (!dealerUid) return;
  // Wipe previous game state to avoid stale results bleeding through
  const currentSnap = await get(rr(dealerUid, 'session/activeGame'));
...
// ============================================================
// 9. resetSession / resetGameOnly
//
// resetSession: wipes only the current game's state.
//   Does NOT clear leaderboard, chat, or presence —
//   those persist across game switches within a session.
//
// resetGameOnly: alias for the same thing (kept for game
//   component compatibility).
// ============================================================
export async function resetSession(dealerUid, gameName) {
  if (!dealerUid) return;
  // Only wipe this game's state — leave leaderboard/chat/presence intact
